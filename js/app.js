...
window.renderAdminDashboard = async () => { 
    const radarAdminHtml = `<div class="bg-white p-5 rounded-xl border"><h4 class="font-bold mb-4 text-sm"><i class="fas fa-satellite-dish text-indigo-600"></i> رادار الرحيبة</h4><div class="grid grid-cols-1 sm:grid-cols-2 gap-3"><div><select id="adminRadarSeason" onchange="setRadarDefaultSeason()" class="ctrl-input text-sm"><option value="summer">صيف</option><option value="winter">شتاء</option></select></div><div><button onclick="resetRadarVotes()" class="bg-red-500 text-white py-2 rounded-lg text-sm w-full">تصفير العدادات</button></div></div></div>`;
    const homeAdsHtml = `<div class="bg-white p-5 rounded-xl border"><h4 class="font-bold mb-4 text-sm"><i class="fas fa-photo-video text-purple-600"></i> إعلانات الصفحة الرئيسية</h4><form onsubmit="saveHomeAd(event)" class="grid grid-cols-1 gap-3 mb-4"><select id="adType" class="ctrl-input text-sm"><option value="image">صورة</option><option value="video">فيديو (mp4)</option></select><input type="text" id="adContent" class="ctrl-input text-sm" placeholder="الرابط..." required><input type="text" id="adLink" class="ctrl-input text-sm" placeholder="رابط التحويل (للصور)"><button type="submit" class="py-2.5 rounded-xl text-white font-semibold text-sm" style="background: #8B5CF6">إضافة</button></form><div id="adminHomeAdsList" class="flex flex-col gap-2 max-h-40 overflow-y-auto"><p class="text-center text-gray-400 text-sm">جاري التحميل...</p></div></div>`;
    const announcementsHtml = `<div class="bg-white p-5 rounded-xl border"><h4 class="font-bold mb-4 text-sm"><i class="fas fa-bullhorn text-blue-600"></i> الشريط الإعلاني</h4><form onsubmit="saveAnnouncement(event)" class="grid grid-cols-1 gap-3 mb-4"><textarea id="annText" class="ctrl-input text-sm" rows="2" placeholder="نص الإعلان" required></textarea><input type="text" id="annLink" class="ctrl-input text-sm" placeholder="الرابط"><button type="submit" class="py-2.5 rounded-xl text-white font-semibold text-sm" style="background: #2563EB">نشر</button></form><div id="adminAnnouncementList" class="flex flex-col gap-2 max-h-40 overflow-y-auto"><p class="text-center text-gray-400 text-sm">جاري التحميل...</p></div></div>`;
    
    let listHtml = allData.map(item => `<div class="flex items-center justify-between p-3 rounded-xl border bg-white"><div><span class="badge badge-${item.type}">${item.type}</span><span class="font-semibold text-sm mr-2">${item.name}</span></div><div class="flex gap-2"><button onclick="editFacility('${item.id}')" class="text-blue-600"><i class="fas fa-edit"></i></button><button onclick="deleteFacility('${item.id}')" class="text-red-600"><i class="fas fa-trash"></i></button></div></div>`).join(''); 

    const twentyHoursAgo = new Date(Date.now() - (20 * 60 * 60 * 1000)).toISOString();
    const activeBloodRequests = bloodRequests.filter(b => b.created_at > twentyHoursAgo);
    let bloodHtml = activeBloodRequests.length === 0 ? '<p class="text-center text-gray-400 text-sm">لا يوجد.</p>' : activeBloodRequests.map(b => `<div class="flex items-center justify-between p-2 rounded-lg border"><div><span class="blood-type-badge text-sm py-1 px-3">${b.blood_type}</span><span class="text-sm mr-2">${b.patient_name}</span></div><button onclick="resolveBloodRequest('${b.id}')" class="text-xs text-white px-3 py-1.5 rounded-lg bg-red-500">إنهاء</button></div>`).join('');
    let medDonHtml = medicineDonations.length === 0 ? '<p class="text-center text-gray-400 text-sm">لا يوجد.</p>' : medicineDonations.map(m => `<div class="flex items-center justify-between p-2 rounded-lg border"><span class="text-sm">${m.medicine_name}</span><button onclick="resolveMedicineDonation('${m.id}')" class="text-xs text-white px-3 py-1.5 rounded-lg bg-red-500">حذف</button></div>`).join('');

    openCtrlPanel('لوحة الإدارة', `<div class="flex flex-col gap-6"> ${announcementsHtml} ${homeAdsHtml} ${radarAdminHtml} <div class="bg-white p-5 rounded-xl border"><h4 class="font-bold mb-4 text-sm"><i class="fas fa-tint text-red-600"></i> استغاثات الدم (${bloodRequests.length})</h4><div class="flex flex-col gap-2 max-h-60 overflow-y-auto">${bloodHtml}</div></div> <div class="bg-white p-5 rounded-xl border"><h4 class="font-bold mb-4 text-sm"><i class="fas fa-hand-holding-medical text-green-600"></i> الأدوية الفائضة (${medicineDonations.length})</h4><div class="flex flex-col gap-2 max-h-60 overflow-y-auto">${medDonHtml}</div></div> <div class="bg-white p-5 rounded-xl border"><h4 class="font-bold mb-4 text-sm"><i class="fas fa-plus-circle ml-2" style="color: var(--accent)"></i> <span id="formTitle">إضافة منشأة</span></h4><form onsubmit="saveFacility(event)" class="grid grid-cols-1 sm:grid-cols-2 gap-3"><input type="hidden" id="edit_id"><select id="new_type" class="ctrl-input text-sm" required onchange="updateAdminFormFields(this.value)"><option value="clinic">عيادة</option><option value="hospital">مشفى</option><option value="center">مركز</option><option value="lab">مخبر</option><option value="doctor">طبيب</option><option value="pharmacy">صيدلية</option></select><input type="text" id="new_name" class="ctrl-input text-sm" placeholder="الاسم" required><input type="text" id="new_specialty" class="ctrl-input text-sm" placeholder="التخصص" required><input type="text" id="new_address" class="ctrl-input text-sm" placeholder="العنوان" required><input type="text" id="new_phone" class="ctrl-input text-sm" placeholder="الهاتف"><input type="text" id="new_hours" class="ctrl-input text-sm" placeholder="أوقات العمل"><input type="text" id="new_image_url" class="ctrl-input text-sm" placeholder="رابط الصورة"><textarea id="new_desc" class="ctrl-input text-sm col-span-2" placeholder="وصف عام" rows="2" required></textarea><div id="adminExtraFields" class="contents"></div><button type="submit" class="col-span-1 sm:col-span-2 py-2.5 rounded-xl text-white font-semibold text-sm" style="background: var(--accent)"><i class="fas fa-save ml-1"></i> حفظ</button></form></div> <div class="bg-white p-5 rounded-xl border"><h4 class="font-bold mb-4 text-sm"><i class="fas fa-list ml-2"></i> المنشآت (${allData.length})</h4><div class="flex flex-col gap-2 max-h-96 overflow-y-auto">${listHtml}</div></div> </div>`, '#073D2E'); 
